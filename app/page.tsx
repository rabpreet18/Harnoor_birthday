...
function DateTable() {
  ...
  const wood = useMemo(() => {
    ...
    for (let y = 0; y < N; y++) {
      ...
      g.fillStyle = `rgba(255,255,255,${(tone / 255) * 0.08})`;
      g.fillRect(0, y, N, 1);
    }
    const t = new THREE.CanvasTexture(c);
    (t as any).colorSpace = THREE.SRGBColorSpace;
...
function Frame({ url, pos = [0, 0, 0], rot = [0, 0, 0], size = [1.5, 1.05] as [number, number] }) {
  ...
  return (
    <group position={pos as any} rotation={rot as any}>
      <RoundedBox args={[w + 0.16, h + 0.16, 0.08]} radius={0.06} smoothness={6} position={[0, h / 2, 0]} castShadow>
      ...
      {!blown && (
        <>
          <sprite ref={flame} position={[0, 0.56, 0]}>
            <spriteMaterial map={flameTex} depthWrite={false} transparent blending={THREE.AdditiveBlending} />
          </sprite>
          <points ref={sparks} position={[0, 0.6, 0]}>
            <primitive attach="geometry" object={sparkGeom} />
            <pointsMaterial size={0.02} sizeAttenuation color="#ffcc66" transparent depthWrite={false} blending={THREE.AdditiveBlending} />
          </points>
        </>
      )}
    </group>
...
function Knife({ active }: { active: boolean }) {
  const { rotY, posX } = useSpring({ rotY: active ? -Math.PI / 3 : -Math.PI / 12, posX: active ? 0.9 : 1.4, config: { mass: 1, tension: 220, friction: 18 } });
  return (
    <animated.group position-z={0.2} position-y={0.82} position-x={posX} rotation-y={rotY}>
      <mesh castShadow>
        <boxGeometry args={[0.9, 0.04, 0.06]} />
        <meshPhysicalMaterial color="#cccccc" metalness={1} roughness={0.2} reflectivity={1} clearcoat={0.6} />
      </mesh>
      ...
      <animated.group rotation-x={rot} position={[0, 0.001, 0]} onClick={onToggle}>
        <Html transform position={[0, 0, 0]} distanceFactor={1.0} /* no occlude so it never hides */>
          <div style={{ width: 740, height: 460, borderRadius: 22, background: "#fff", boxShadow: "0 26px 44px rgba(0,0,0,0.28)", padding: 30, cursor: "pointer", fontFamily: 'ui-sans-serif, system-ui, -apple-system, "Segoe UI", Roboto, "Helvetica Neue", Arial' }}>
            ...
            <div style={{ marginTop: 12, opacity: 0.6, fontSize: 15 }}>(Click to {open ? "close" : "open"})</div>
          </div>
        </Html>
      </animated.group>
    </group>
  ...
  return (
    <>
      ...
      <Suspense fallback={null}>
        ...
        <Frame url={PHOTOS[1]} pos={[-1.1, 0.09, -1.35]} rot={[0, THREE.MathUtils.degToRad(42), 0]} />
  ...
  return (
    <div style={{ width: "100vw", height: "100vh", background: "#0b0b0b" }}>
      ...
      <div style={{ position: "absolute", top: 14, right: 14, zIndex: 15, display: "flex", gap: 10 }}>
        ...
        <button onClick={() => { if (!player.current) return; if (playing) player.current.pauseVideo(); else player.current.playVideo(); setPlaying((p) => !p); }}
          style={{ padding: "12px 16px", background: "rgba(255,255,255,.18)", color: "#fff", border: 0, borderRadius: 12, cursor: "pointer", fontSize: 15, backdropFilter: "blur(6px)" }}>
          {playing ? "Pause" : "Play"}
        </button>
      </div>
