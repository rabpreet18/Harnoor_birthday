 ...
 * - Click the candle to blow it out (or press Space)
 * - Click the cake to cut a slice
 * - Click the birthday card to open/close
 * - Upload custom photos and set a custom background
 * - YouTube/YouTube Music audio with autoplay (after tap), mute & play/pause
 */

const DEFAULT_PHOTOS = [
  "https://drive.google.com/file/d/1V-7-g4w2gL-4LQjw0Z2F4iyR3hg7PJX3/view?usp=drive_link",
  "https://drive.google.com/file/d/1tUfkOGbT-MRMQFYrrM89M2MPD3cnLI3m/view?usp=drive_link",
  "https://drive.google.com/file/d/19KqIe21G_pySxTik2WW_m7r9m-ioCbMT/view?usp=drive_link",
  "https://drive.google.com/file/d/1gQfAZ_JQ0kXvQiB4wxl0QjQnsGN22uT3/view?usp=drive_link",
];
...
function PhotoFrame({ url, position = [0, 0, 0], rotation = [0, 0, 0], size = [1.4, 1] }) {
...
function Candle({ blown, onToggle }) {
...
function Cake({ onCut, cut, blown, setBlown }) {
  ...
  const { sliceX, topY } = useSpring({
    sliceX: cut ? 1.2 : 0, // move slice out
    topY: cut ? 0.02 : 0.08, // slightly sink top on cut
  ...
  return (
    <group position={[0, 0.5, 0]}>
      ...
      <animated.group position-x={sliceX}>
        <mesh castShadow position={[0, 0.38, 0]}>
          {/* wedge-shaped slice using cylinder thetaLength */}
...
function BirthdayCard({ open, onToggle, title, message, signature }) {
  ...
  return (
    <>
      ...
      <Suspense fallback={null}>
        ...
        <Cake onCut={() => setCut((c) => !c)} cut={cut} blown={blown} setBlown={setBlown} />
        {/* frames around the cake */}
        ...
        <BirthdayCard open={state.cardOpen} onToggle={() => state.setCardOpen((o) => !o)} title={card.title} message={card.message} signature={card.signature} />
...
export default function BirthdayShowcase() {
  ...
  const [bgUrl, setBgUrl] = useState("https://images.unsplash.com/photo-1505761671935-60b3a7427bad?q=80&w=1200&auto=format&fit=crop");
  ...
  const [showGate, setShowGate] = useState(false); // user gesture gate for autoplay policies
  ...
  // URL params for quick share links: t,m,s (card), bg, p1..p4 (photos), yt (YouTube link/ID)
  React.useEffect(() => {
    try {
      const params = new URLSearchParams(window.location.search);
      const t = params.get("t"), m = params.get("m"), s = params.get("s");
      const bg = params.get("bg");
      const p = [params.get("p1"), params.get("p2"), params.get("p3"), params.get("p4")].filter(Boolean);
        ...
        <div style={{ background: "rgba(20,20,20,0.6)", backdropFilter: "blur(8px)", borderRadius: 14, padding: 12 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            ...
            <input
              ...
              onChange={(e) => setCard({ ...card, signature: e.target.value })}
              placeholder="Signature"
            />
          </div>
          <textarea
            style={{ ...inputStyle, marginTop: 8, height: 72 }}
            value={card.message}
        ...
        <div style={{ position: "absolute", width: 0, height: 0, overflow: "hidden" }}>
          <YouTube
            ...
            opts={{
              height: "0",
              width: "0",
              playerVars: { autoplay: 1, mute: isMuted ? 1 : 0, loop: 1, playlist: ytId },
            }}
            onReady={(e) => {
              playerRef.current = e.target;
              if (showGate) { e.target.pauseVideo(); } else { setIsPlaying(true); }
            }}
            onEnd={(e) => { e.target.seekTo(0); e.target.playVideo(); }}
          />
        </div>
      )}
